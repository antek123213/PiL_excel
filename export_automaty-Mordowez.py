...
def _resolve_lista_automatow_file_for_month(month_str, lista_file=None):
    ...
    _dt = datetime.strptime(month_str, '%Y-%m')
    candidates = [
...
def _get_monthly_costs_per_device(
    ...
    global_costs['infolinia'] = INFOLINIA_MONTHLY_COST if month_str >= INFOLINIA_START_MONTH else 0.0
    ...
    for device_id in normalized_ids:
        ...
        # OH liczymy zgodnie z nowa reguła: 0.2 * (NON TVM + Project Variable Costs)
        per_device['oh'] = (
...
def _format_worksheet_columns(ws, padding_ratio=0.10, minimum_widths=None, max_width=50):
    """
    Dostosowuje szerokości kolumn arkusza na podstawie długości tekstów.
    Domyślnie dodaje 10% zapasu.
    """
    ...
    for col_idx, col in enumerate(ws.iter_cols(1, ws.max_column), start=1):
        ...
        for cell in col:
            try:
                ...
                if not text:
                    continue
                # Kolumny szerokie są liczone głównie z tekstów nagłówków i opisów.
                # Liczb nie używamy jako głównego wyznacznika szerokości.
                if isinstance(value, (int, float)):
                    continue
        ...
        if max_length == 0:
            ...
            if min_width > 0:
                ws.column_dimensions[column].width = min(min_width, max_width)
            continue

        width = math.ceil(max_length * (1.0 + padding_ratio))
        width = max(width, minimum_widths.get(column, 0))
        ws.column_dimensions[column].width = min(width, max_width)
...
def export_to_excel_PL(
    ...
    podsum_cols = {
        ...
        'profit_loss': podsum_start_col + 7,
    }
    podsum_end_col = podsum_start_col + 7
    ...
    if last_data_row >= 2:
        ...
        for col_num in numeric_columns:
            ...
            if col_num in ticket_columns.values():
    ...
    minimum_widths = {
        get_column_letter(num1_col): 4,
        get_column_letter(info_nr_col): 10,
        get_column_letter(info_loc_col): 42,
        get_column_letter(num2_col): 4,
        get_column_letter(num3_col): 4,
        get_column_letter(num4_col): 4,
        get_column_letter(num5_col): 4,
        get_column_letter(num6_col): 4,
        get_column_letter(num7_col): 4,
        get_column_letter(uwagi_col_idx): 22,
    }
    for code in carrier_order:
        cols = carrier_columns[code]
        minimum_widths[get_column_letter(cols['brutto'])] = 12
        minimum_widths[get_column_letter(cols['prowizja'])] = 12
        minimum_widths[get_column_letter(cols['netto'])] = 12
        minimum_widths[get_column_letter(cols['gotowka'])] = 11
        minimum_widths[get_column_letter(cols['karta'])] = 11
        minimum_widths[get_column_letter(cols['blik'])] = 10
    for col in ticket_columns.values():
        minimum_widths[get_column_letter(col)] = 10
    for col in [przychody_cols['prowizja_suma'], przychody_cols['interchange'], przychody_cols['dodatkowe_zyski']]:
        minimum_widths[get_column_letter(col)] = 14
    for col in list(tvm_cost_cols.values()) + [koszty_tvm_col] + list(other_cost_cols.values()) + [dodatkowe_koszty_col]:
        minimum_widths[get_column_letter(col)] = 12
    for col in podsum_cols.values():
        minimum_widths[get_column_letter(col)] = 13

    _format_worksheet_columns(ws, padding_ratio=0.10, minimum_widths=minimum_widths)
...
def _build_month_export_payload_with_retry(
    ...
    while attempt <= max_attempts:
        try:
            ...
            try:
                reconnect_db_name = conn.info.dbname if conn is not None else None
            except Exception:
                reconnect_db_name = None
            ...
            try:
                conn = connect_to_db(
                    database_name=reconnect_db_name,
                    max_retries=5,
                    retry_delay=8,
                )
