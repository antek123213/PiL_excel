...
DEFAULT_LISTA_AUTOMATOW_FILE = Path(__file__).parent / 'lista_automatow.xlsx'
...
def export_to_excel_PL(
    ...
    ws.column_dimensions[get_column_letter(num1_col)].width = 5
    ws.column_dimensions[get_column_letter(info_nr_col)].width = 12
    ws.column_dimensions[get_column_letter(info_loc_col)].width = 42
    ws.column_dimensions[get_column_letter(info_type_col)].width = 18
    ws.column_dimensions[get_column_letter(num2_col)].width = 5
    for code in carrier_order:
        cols = carrier_columns[code]
        ws.column_dimensions[get_column_letter(cols['brutto'])].width = 13
        ws.column_dimensions[get_column_letter(cols['prowizja'])].width = 13
        ws.column_dimensions[get_column_letter(cols['netto'])].width = 13
        ws.column_dimensions[get_column_letter(cols['gotowka'])].width = 12
        ws.column_dimensions[get_column_letter(cols['karta'])].width = 12
        ws.column_dimensions[get_column_letter(cols['blik'])].width = 11
    ws.column_dimensions[get_column_letter(num3_col)].width = 5
    for col in ticket_columns.values():
        ws.column_dimensions[get_column_letter(col)].width = 11
    ws.column_dimensions[get_column_letter(num4_col)].width = 5
    for col in [przychody_cols['prowizja_suma'], przychody_cols['interchange'], przychody_cols['dodatkowe_zyski']]:
        ws.column_dimensions[get_column_letter(col)].width = 15
    ws.column_dimensions[get_column_letter(num5_col)].width = 5
    for col in list(tvm_cost_cols.values()) + [koszty_tvm_col] + list(other_cost_cols.values()) + [dodatkowe_koszty_col]:
        ws.column_dimensions[get_column_letter(col)].width = 13
    ws.column_dimensions[get_column_letter(num6_col)].width = 5
    for col_name, col_num in podsum_cols.items():
        if col_name in {'koszty', 'przychody', 'brutto_suma', 'netto_suma', 'karta_suma', 'blik_suma', 'gotowka_suma', 'bilety_suma', 'profit_loss'}:
            ws.column_dimensions[get_column_letter(col_num)].width = 15
    ws.column_dimensions[get_column_letter(num7_col)].width = 5
    ws.column_dimensions[get_column_letter(uwagi_col_idx)].width = 24
...
def _create_tickets_summary_sheet(wb, month_to_tickets):
    ...
    # Adjust column widths (10% extra margin + minimum widths for readability)
    for col in range(1, len(headers) + 1):
        ...
        ws.column_dimensions[get_column_letter(col)].width = max(max_len * 1.1, min_width)
